...
class RandomFlip(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            sample['flipped'] = not sample['flipped']
            sample['image'] = im
        return sample
...
class Pad(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if self.im_shape:
            sample['im_shape'] = np.asarray([sample['image'].shape[0], sample['image'].shape[1]], dtype=np.float32)
        if self.pad_mode == 0:
            return sample
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
...
@register_op
class ChiLunResize(BaseOperator):
    def __init__(self, base_ratio=1):
        super(ChiLunResize, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if h == 1500:
            ...
            raise RuntimeError('img shape error')
        target_size = [round(i * self.base_ratio) for i in target_size]
        resizer = Resize(target_size, False, interp=cv2.INTER_CUBIC)
...
@register_op
class ChiLunResizeInvert(BaseOperator):
    def __init__(self, base_ratio=1):
        super(ChiLunResizeInvert, self).__init__()
        self.base_ratio = base_ratio

    def apply(self, sample, context=None):
        im = sample['image']
        h, w, _ = im.shape
        if h == 1902 and w == 1076:
            target_size = [1500, 1000]
        elif h == 1500 and w == 841:
            target_size = [1500, 1400]
        elif h == 3354 and w == 1262:
            target_size = [2000, 1400]
        else:
            raise RuntimeError(h, w)
        target_size = [round(i * self.base_ratio) for i in target_size]
        resizer = Resize(target_size, False, interp=cv2.INTER_CUBIC)
        sample = resizer(sample, context=context)
        # im = sample['image']
...
class RandomSizeCrop(BaseOperator):
    ...
    def __init__(self, target_size, scale, max_ratio=1, object_prob=0):
    ...
    def get_crop_params(self, img_shape, output_size, gt_bbox):
        ...
        if w == tw and h == th:
            return 0, 0, h, w
        if random.uniform(0, 1) < self.object_prob and len(gt_bbox) > 0:
            id = random.randint(0, len(gt_bbox) - 1)
            x_min, y_min, x_max, y_max = gt_bbox[id, :].astype(np.int32)
            x_lb = max(min(x_min, x_max - tw), 0)
            x_ub = min(max(x_min, x_max - tw), w - tw)
            y_lb = max(min(y_min, y_max - th), 0)
            y_ub = max(min(y_min, y_max - th), h - th)
            i = random.randint(y_lb, y_ub)
            j = random.randint(x_lb, x_ub)
        else:
    ...
    def apply(self, sample, context=None):
        # size = self.target_size / (self.scale ** random.uniform(-1, 1))
        size = self.target_size / (random.uniform(2 - self.scale, self.scale))
        ratio = random.uniform(1, self.max_ratio)
        if random.randint(0, 1):
            h = min(sample['image'].shape[0], int(size / math.sqrt(ratio)))
            w = min(sample['image'].shape[1], int(size * math.sqrt(ratio)))
        else:
            h = min(sample['image'].shape[0], int(size * math.sqrt(ratio)))
            w = min(sample['image'].shape[1], int(size / math.sqrt(ratio)))
        region = self.get_crop_params(sample['image'].shape[:2], [h, w], sample['gt_bbox'])
...
class RandomSizeCrop(BaseOperator):
    ...
    def __init__(self, target_size, scale, max_ratio=1,
                 thresholds=[.0, .1, .3, .5, .7, .9],
                 num_attempts=50,
                 allow_no_crop=False,
                 cover_all_box=False,
                 filter_center_out=True,
                 is_mask_crop=False,
                 fill_value=[0, 0, 0]):
        super(RandomSizeCrop, self).__init__()
        assert isinstance(fill_value, (Number, Sequence)), \
        ...
        self.cover_all_box = cover_all_box
        self.is_mask_crop = is_mask_crop
        self.filter_center_out = filter_center_out
    ...
    def apply(self, sample, context=None):
        ...
        for thresh in thresholds:
            ...
            found = False
            for i in range(self.num_attempts):
                temp = (random.uniform(2 - self.scale, self.scale))
                target_h = self.target_size[0] / temp
                target_w = self.target_size[1] / temp
                ratio = random.uniform(1, self.max_ratio)
                if random.randint(0, 1):
                    crop_h = int(target_h / math.sqrt(ratio))
                    crop_w = int(target_w * math.sqrt(ratio))
                else:
                    crop_h = int(target_h * math.sqrt(ratio))
                    crop_w = int(target_w / math.sqrt(ratio))
                if h < crop_h or w < crop_w:
                    sample = RandomSizeExpand(None, 1.1, [crop_h, crop_w], self.fill_value)(sample)
                    h, w = sample['image'].shape[:2]
                    gt_bbox = sample['gt_bbox']
...
@register_op
class RandomSizeExpand(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, size=None, prob=0.5, target_size=None, fill_value=(127.5, 127.5, 127.5)):
        super(RandomSizeExpand, self).__init__()
        self.size = size
        self.prob = prob
        assert size is None or target_size is None
        assert isinstance(fill_value, (Number, Sequence)), \
            "fill value must be either float or sequence"
        if isinstance(fill_value, Number):
            fill_value = (fill_value,) * 3
    ...
    def apply(self, sample, context=None):
        ...
        if self.size is not None:
            H = np.random.randint(0, self.size + 1) + height
            W = np.random.randint(0, self.size + 1) + width
        else:
            H = max(self.target_size[0], height)
            W = max(self.target_size[1], width)
        left = np.random.randint(0, W - width + 1)
        top = np.random.randint(0, H - height + 1)
        offsets, size = [left, top], [H, W]
        pad = Pad(size,
                  pad_mode=-1,
                  offsets=offsets,
                  fill_value=self.fill_value)
...
class SizeExpand(BaseOperator):
    ...
    def __init__(self, size=4., fill_value=(127.5, 127.5, 127.5)):
        super(SizeExpand, self).__init__()
        assert size > 0, "expand must be larger than 0"
        self.size = size
    ...
    def apply(self, sample, context=None):
        im = sample['image']
        height, width = im.shape[:2]
        offsets, size = [self.size, self.size], [height + self.size * 2, width + self.size * 2]
        pad = Pad(size,
                  pad_mode=-1,
                  offsets=offsets,
                  fill_value=self.fill_value)
...
@register_op
class MultiRatioTestResize(BaseOperator):
    def __init__(self, target_ratio=[],
                 interp=cv2.INTER_LINEAR, use_flip_updown=False, use_rot=False,
                 use_flip=False, use_flip_updownleftright=False):
        """
        Rescale image to the each size in target size, and capped at max_size.
        Args:
            origin_target_size (list): origin target size of image
            ...
            use_flip (bool): whether use flip augmentation.
        """
        super(MultiRatioTestResize, self).__init__()
        self.target_ratio = target_ratio
        self.interp = interp
        self.use_flip = use_flip
        self.use_flip_updown = use_flip_updown
        self.use_flip_updownleftright = use_flip_updownleftright
    ...
    def apply(self, sample, context=None):
        ...
        im = sample['image']
        height, width = im.shape[:2]
        for ratio in self.target_ratio:
            size = [int(ratio * height), int(ratio * width)]
            resizer = Resize(size, keep_ratio=True, interp=self.interp)
            samples.append(resizer(sample.copy(), context))
        # print(samples[0]['image'][100:104,100,0])
        # print(sample['image'].max())
        if self.use_flip:
            flipper = RandomFlip(1.1)
            s_new = []
            for s in samples:
                s_new.append(flipper(s.copy(), context=context))
            samples.extend(s_new)
        if self.use_flip_updown:
            flipper = RandomFlipUpDown(1.1)
            s_new = []
            for s in samples:
                s_new.append(flipper(s.copy(), context=context))
            samples.extend(s_new)
        if self.use_flip_updownleftright:
            flipper = RandomFlipUpDownLeftRight(1.1)
            s_new = []
            for s in samples:
                s_new.append(flipper(s.copy(), context=context))
            samples.extend(s_new)
        if self.use_rot:
            ...
            s_new = []
            for s in samples:
                s_new.append(rotter(s.copy(), context=context))
            samples.extend(s_new)
        # flipper = RandomFlipUpDown(1.1)
        # samples = [RandomFlipUpDownLeftRight(1.1)(samples[0].copy(), context=context),]
        # print(samples[0]['image'][100:104,100,0], samples[1]['image'][100:104,100,0],im[100:104,100,0])
        # print(samples[0]['image'].max(), samples[1]['image'].max())
        # samples.pop(0)
        return samples


@register_op
class RandomFlipUpDown(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(RandomFlipUpDown, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            sample['flippedupdown'] = not sample['flippedupdown']
            sample['image'] = im
        return sample


@register_op
class RandomFlipUpDownLeftRight(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(RandomFlipUpDownLeftRight, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply(self, sample, context=None):
        ...
        """
        if np.random.uniform(0, 1) < self.prob:
            a = RandomFlipUpDown(1.1)
            b = RandomFlip(1.1)
            sample = a(b(sample, context), context)
        return sample


@register_op
class Rot(BaseOperator):
    def __init__(self, ):
        """
        Args:
            prob (float): the probability of flipping image
    ...
    def apply(self, sample, context=None):
        ...
        sample['rot'] = not sample['rot']
        sample['image'] = im
        sample['im_shape'] = sample['im_shape'][::-1]
        sample['scale_factor'] = sample['scale_factor'][::-1]
        return sample


@register_op
class RandomRatioResize(BaseOperator):
    def __init__(self, ratio, interp=cv2.INTER_LINEAR, ):
        """
        Rescale image to the each size in target size, and capped at max_size.
        Args:
            origin_target_size (list): origin target size of image
            ...
            use_flip (bool): whether use flip augmentation.
        """
        super(RandomRatioResize, self).__init__()
        self.interp = interp
        self.ratio = ratio
    ...
    def apply(self, sample, context=None):
        ...
        target_size = [round(h * ratio), round(w * ratio), ]
        resizer = Resize(target_size, False)
        return resizer(sample, context=context)


@register_op
class FilterObject(BaseOperator):
    """Resize image and bbox based on long side (with optional random scaling),
       then crop or pad image to target size.
    Args:
        target_dim (int): target size.
    ...
    def __init__(self, area_range=(16, 100000000000), class_id=None):
    ...
    def apply(self, sample, context=None):
        ...
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            ...
            if self.class_id is not None:
                class_valid = (sample['gt_class'][:, 0] == self.class_id[0])
                for i in self.class_id[1:]:
                    class_valid = class_valid + (sample['gt_class'][:, 0] == i)
                keep_index = (area > self.area_range[0]) * (area < self.area_range[1]) * class_valid
            else:
                keep_index = (area > self.area_range[0]) * (area < self.area_range[1])
            # mask = area <= self.area_range[0]
            # boxes_small = boxes[mask, :]
            # im = sample['image']
            ...
            #     x_min = max(x_min, 0)
            #     y_min = max(y_min, 0)
            #     x_max = min(x_max, im.shape[1])
            #     y_max = min(y_max, im.shape[0])
            #     im[y_min:y_max, x_min:x_max, :] = 0
            # sample['image'] = im
            sample['gt_bbox'] = boxes[keep_index, :]
...
@register_op
class Canny(BaseOperator):
    def __init__(self, p=(100, 150, 200, 250)):
        """
        Rescale image to the each size in target size, and capped at max_size.
        Args:
            origin_target_size (list): origin target size of image
    ...
    def apply(self, sample, context=None):
        ...
        r1 = cv2.Canny(gray, self.p[0], self.p[1])
        # im[:, :, 1] = r1
        r2 = cv2.Canny(gray, self.p[2], self.p[3])
        im = im.copy()
        im[:, :, 1] = r1
        im[:, :, 2] = r2
        sample['image'] = im.astype(np.float32)
