...
class CascadeRCNN(BaseArch):
    ...
    def _forward(self):
        ...
        if self.training:
            rois, rois_num, rpn_loss = self.yolo_head(body_feats, self.inputs)
            ...
            rois, rois_num, score = self.yolo_head(body_feats, self.inputs)
            ...
            #bbox, bbox_num = self.bbox_post_process(preds, (refined_rois, rois_num), score, im_shape, scale_factor)
            bbox, bbox_num = self.bbox_post_process(preds, (refined_rois, rois_num), im_shape, scale_factor)
            # rescale the prediction back to origin image
            bbox, bbox_pred, bbox_num = self.bbox_post_process.get_pred(
                bbox, bbox_num, im_shape, scale_factor,
              #  flipped=self.inputs['flipped'], flippedupdown=self.inputs['flippedupdown'], rot=self.inputs['rot']
            )
    ...
    def get_loss(self, ):
        ...
        rpn_losses = rpn_loss.pop('loss')

        if self.with_mask:
            loss.update(mask_loss)
        total_loss = paddle.add_n(list(loss.values())) + rpn_losses
