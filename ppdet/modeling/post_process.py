...
__all__ = [
    ...
    'SparsePostProcess', 'MyBBoxPostProcess'
]


@register
class MyBBoxPostProcess(object):
    __shared__ = ['num_classes', 'export_onnx', 'export_eb']
    __inject__ = ['decode', 'nms']

    def __init__(self, num_classes=80, decode=None, nms=None,
                 export_onnx=False, export_eb=False):
        super(MyBBoxPostProcess, self).__init__()
    ...
    def __call__(self, head_out, rois, rpn_label_score, im_shape, scale_factor):
        """
        ...
        if self.nms is not None:
            ...
            # print(bboxes)
            # print(rpn_label_score)
            #print(score)
            #print(rpn_label_score[0][:, 1])
            #score = score * rpn_label_score[0][:, 1:]
            score = score * rpn_label_score[0]
            #score = 0.5 * (score + rpn_label_score[0])
            #score = (score + rpn_label_score[0][:, 1:])*0.5
            #print(paddle.concat([paddle.max(score[:, :-1], -1, keepdim=True), rpn_label_score[0][:, 1:]], 1))
            #print(paddle.concat([score, rpn_label_score[0]], 1))
            #print(rpn_label_score[0])
            bbox_pred, bbox_num, _ = self.nms(bboxes, score, self.num_classes)
            #print(rois)
            #print(bboxes, bbox_pred.shape)
            #print(rpn_label_score[0].shape, bboxes[0][:, 0, :].shape)
            #print(bbox_num)

            #bbox_pred = paddle.concat([rpn_label_score[0], bboxes[0][:, 0, :]], axis=1)
            #bbox_pred = paddle.concat([rpn_label_score[0], rois[0][0]], axis=1)
            #bbox_num = paddle.to_tensor([bboxes[0].shape[0],], dtype=paddle.int32)
    ...
    def get_pred(self, bboxes, bbox_num, im_shape, scale_factor, flipped, flippedupdown, rot):
        """
        ...
        if rot:
            x_min, y_min, x_max, y_max = paddle.split(pred_bbox, 4, axis=1)
            pred_bbox = paddle.concat([y_min, x_min, y_max, x_max], axis=1)
            height, width = width, height
        if flipped:
            x_min = pred_bbox[:, 0].clone()
            x_max = pred_bbox[:, 2].clone()
            pred_bbox[:, 0] = width - x_max
            pred_bbox[:, 2] = width - x_min
        if flippedupdown:
            y_min = pred_bbox[:, 1].clone()
            y_max = pred_bbox[:, 3].clone()
            pred_bbox[:, 1] = height - y_max
            pred_bbox[:, 3] = height - y_min
        # '''
...
class BBoxPostProcess(object):
    ...
    def get_pred(self, bboxes, bbox_num, im_shape, scale_factor, **kwargs):
        """
        Rescale, clip and filter the bbox from the output of NMS to 
        get final prediction. 
