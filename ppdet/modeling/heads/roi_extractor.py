...
class RoIAlign(nn.Layer):
    ...
    def __init__(self,
        ...
        if out_channel > 0:
            ...
            for in_c in in_channel:
                self.conv.append(ConvBNLayer(in_c, out_channel, 1, act='swish'))
    ...
    def from_config(cls, cfg, input_shape):
        ...
        return {'spatial_scale': [1. / i.stride for i in input_shape],
                'in_channel': [i.channels for i in input_shape]
                }

    def __call__(self, feats, roi, rois_num):
        if self.out_channel > 0:
            feats = [j(i) for i, j in zip(feats, self.conv)]
    ...
    @property
    def out_shape(self):
        return [ShapeSpec(channels=c if self.out_channel == 0 else self.out_channel, stride=1.0 / s) for c, s in
                zip(self.in_channel, self.spatial_scale)]


@register
class RoIAlignMask(RoIAlign):
    pass
