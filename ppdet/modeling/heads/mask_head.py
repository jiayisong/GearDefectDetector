...
class MaskHead(nn.Layer):
    ...
    def __init__(self,
        ...
        if feat_channel > 0:
            ...
            for in_c in fpn_channel:
                self.conv.append(ConvBNLayer(in_c, feat_channel, 1, act=None))
    ...
    def from_config(cls, cfg, input_shape):
        ...
        return {
            ...
            'head': head,
            'fpn_channel': [i.channels for i in input_shape]
        }
    ...
    def forward(self,
                ...
                bbox_feat=None,
                feat_func=None):
        if self.feat_channel > 0:
            for i, j in enumerate(zip(body_feats[::-1], self.conv[::-1])):
                if i == 0:
                    body_feat = j[1](j[0])
                else:
                    body_feat = body_feat + F.interpolate(j[1](j[0]), scale_factor=2. ** i, mode='nearest', )
            body_feats = [body_feat, ]
        if self.training:
