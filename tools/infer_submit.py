...
def run(FLAGS, cfg):
    ...
    # 保存id和路径的对照文件
    with open(os.path.join(FLAGS.output_dir, 'id2path.json'), 'w') as f:
        json.dump(trainer.dataset.get_imid2path(), f)


def transform_format(FLAGS):
    with open(os.path.join(FLAGS.output_dir, 'bbox.json'), 'r') as f1:
        results = json.load(f1)
    with open(os.path.join(FLAGS.output_dir, 'id2path.json'), 'r') as f2:
    ...
    # 生成上传文件
    with open(os.path.join(FLAGS.output_dir, 'upload.json'), 'w') as f:
